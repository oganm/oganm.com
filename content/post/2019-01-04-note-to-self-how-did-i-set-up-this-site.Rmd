...
* [Font-awesome (and emojis)](#font-awesome-and-emojis)
* [[[menu.main]] Children](#menu.main-children)
* [A favicon](#a-favicon)
* [Category links](#category-links)
* [Google analytics](#google-analytics-and-disqus)
* [Homepage](#homepage)
...
Viewing the site for the first time, it looked horrible because images and css did not
...
Well I won't have to revisit that issue now yay. I then delete the files for the default posts. 
...
add a space after `heart` to make it visible for now. I wonder what happens if I 
...
<details><summary>Hidden here is a rabbithole I got into that lead nowhere. Click at your own peril. I will now pretend this never happened.</summary>
...
That means I am using out of date fontawesome but I can deal with that later. Or maybe I'll just switch to "hugo-academic". I am an academic after all. I delete  the `assets.toml` before  I leave to not have leftovers.
</details>

As anyone who knows what they are doing would do, I went to [this](https://fontawesome.com/start) site to get the code required to enable font-awesome. I created `layouts/partials/head_custom.html` and pasted the line in it. It just worked. Easy peasy. Note that I deviate from the slides a little here as I use the latest version of font-awesome. 

Here's an igloo: <i class="fas fa-igloo"></i>

In order to enable using font-awesome at the `[[menu.main]]`s however I need to modify `nav.html` as well. I will just get that from the 
...
So let's dispense with the shorcuts here and change `nav.html` to be
...
And it just works. Great. Now we can get rid of the names to leave the icons. Actually we can't
because it seems names are important in differentiating the menus. When I delete both of their names,
one of them disappear which is not ideal. Looking at [someone else's](https://github.com/mikmart/mikkomarttila/blob/master/config.toml)
`config.toml`, I infer that the name can be the same if you provide `weight`s. Probably
because they are sorted based on their names if a `weight` isn't provided. Nope that didn't fix it.
...
should do the trick, alas it doesn't work. It seems the default bookdown theme doesn't support dropdown menus. Let's look at a theme that does and see if we can
steal the [relevant code](https://github.com/gcushen/hugo-academic/blob/6cac50ecd6f97b52c4d38ff6f351ef1cace891d2/layouts/partials/navbar.html#L27-L67). I can't just copy and paste this code as it seems to use the menu item properties differently than the default theme. However just looking at the code I have a better understanding of how hugo is supposed to work. I see structures like loops 
...
Nothing should change. If the menu item has a child just don't do anything and that's exactly what I see.
...
lift the entire css file and will tweak later if needed. I create a `dropdown.css` file
...
Now this "works" but the coloring placement of the item is very wrong. This should be solvable 
by fiddling with the css. To simplify, I delete the `.navbar` classes. The `.dropdown` seems to be configured
to float to left so I change that to right. The `.dropbtn` has all sorts of properties like padding, font size etc.
I imagine it will work fine if it just inherit those from it's parent so I delete them. It stats looking like it belongs now. I change the background color of the hovering to a respectable gray from that horrible red. It is almost perfect now.
...
they don't align together. Examining the element reveals the issue: the theme. While 
...
a missing html closing time from the time I wrote the dropdown menu code so I go back to `nav.html`.
...
![](/images/correctNest.PNG)


Wait a minute no it's not. Why did it place the tools tab before blog tab. Weren't the weights
...
`config.toml`'s `googleAnalytics` setting. However when I tested it there was no traffic seen. After a short googling I remembered I use browser plugins to stop google from tracking me. I was happy to see that it works on other browsers.
...
This means I in my `disqus.Rmd` I can set `disqus: false` using the yaml and discus section should disappear... And it did. So I guess I am done for now. If I change anything else I will return...