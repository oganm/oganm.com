---
title: 'Is your D&D character rare? II: Off-brand edition'
author: Ogan Mancarci
date: '2018-07-28'
slug: is-your-d-d-character-rare-ii-off-brand-edition
output:
  blogdown::html_page:
    toc: true
categories:
...
charTable = read_tsv(here("dndstats/charTable.tsv"),na = 'NA')
...
write_tsv(uniqueTable,path = here('dndstats/uniqueTable.tsv'))
...
fiveThirtyEight = read_tsv(here('dndstats/538.tsv')) %>% melt()
...
c('Wizard','Cleric','Sorcerer','Druid','Warlock','Bard') %>% lapply(function(x){
    ...
    saveRDS(frame,here('debug.rds'))
    
    kbl = kable(frame,caption = x,format = 'html',escape = FALSE) %>% 
        kable_styling("striped", full_width = F) 
    writeLines(kable(frame,caption = x,format = 'html'),here('debug.txt'))

    for(i in seq_along(groupLevels)){
        kbl %<>% group_rows(groupLevels[i],groupSep[i],groupSep[i+1]-1)
    }
    
    kbl %>%  scroll_box(width = "100%", height = "250px") %>% remove_table_numbers() %>% HTML()
}) -> tables
div(
    fluidRow(
        column(4,
